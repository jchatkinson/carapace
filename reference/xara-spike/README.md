...
design — see `docs/xara-feasibility.md` for the design conclusions drawn
from this spike, and `docs/implementation-plan.md` §5, §6, §8 for where
those conclusions feed into the current plan. Build artifacts (`.o` files,
logs) were intentionally not copied — regenerate them by following the
steps in the implementation plan / conversation history if needed; they're
...
- **`arpack_spike/`** — proved `f2c`-translating Xara's *legacy* `OTHER/ARPACK`
  ...
  §5) to WebAssembly produces correct results for `dsaupd`/`dseupd`, verified
...
- **`poc/`** — the full Truss+ElasticMaterial static-analysis wasm POC, using
  ...
  `XaraClassBroker` eager-construction problem (`docs/xara-feasibility.md` #1).
  - `src/main.cpp` — the driver. This is what `docs/implementation-plan.md`
    §4.4's typestate `AnalysisBuilder` sketch is informed by: it replicates
  ...
  - `patched/BasicAnalysisBuilder.{h,cpp}` and `ProcessContext.h` — **unmodified
    ...
    class directly (see `docs/xara-feasibility.md` #1).
  - `patched/cmx.cpp` — a Gauss-Jordan stand-in for Xara's missing
    ...
    wasm — see `docs/xara-feasibility.md` #4).
  ...
  - `cpplist2.txt` — the resolved 97-file dependency closure for
    ...
    to 348 files — see `docs/xara-feasibility.md` #1).
