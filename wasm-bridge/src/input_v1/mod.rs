...
//! `Session`/`advance` API. It does not yet cross the `wasm_bindgen`/JS
//! boundary — these are plain Rust value types today, not the
//! structured-clone-plus-transferable-typed-array shapes `postMessage`
//! will carry. That binding is deferred until `pysees`'s compiler exists
//! to actually produce `CarapaceInputV1` bytes; see each table's doc
//! comment for the exact correspondence once it does.
